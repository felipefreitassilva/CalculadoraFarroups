...
export default function media2ano() {
	...
	function Gradeful() {
		const [mediaFinalPortugues, setMediaFinalDePortugues] = useState<
			string | number
		>('')
		const [mediaFinalProducaoTextual, setMediaFinalProducaoTextual] =
			useState<string | number>('')
		const [mediaFinalLiteratura, setMediaFinalLiteratura] = useState<
			string | number
		>('')
		const [mediaFinalArtes, setMediaFinalArtes] = useState<string | number>(
			''
		)
		const [mediaFinalEducacaoFisica, setMediaFinalEducacaoFisica] =
			useState<string | number>('')
		...
		const mediaFinalGeral =
			(parseFloat(mediaFinalPortugues.toString()) +
				parseFloat(mediaFinalProducaoTextual.toString()) +
				parseFloat(mediaFinalLiteratura.toString()) +
				parseFloat(mediaFinalArtes.toString()) +
				parseFloat(mediaFinalEducacaoFisica.toString()) +
				parseFloat(mediaFinalIngles.toString()) +
				parseFloat(mediaFinalAlemao.toString()) +
				parseFloat(mediaFinalHistoria.toString()) +
				parseFloat(mediaFinalGeografia.toString()) +
				parseFloat(mediaFinalFilosofia.toString()) +
				parseFloat(mediaFinalSociologia.toString()) +
				parseFloat(mediaFinalAtualidades.toString()) +
				parseFloat(mediaFinalFisica.toString()) +
				parseFloat(mediaFinalQuimica.toString()) +
				parseFloat(mediaFinalBiologia.toString()) +
				parseFloat(mediaFinalMatematica.toString())) /
				16 || 0
		...
		const materias = [
			...
			},
			{
				...
				content: (
					<div className={styles.wrapper}>
						<h2 className={styles.materia}>Historia</h2>
						<div className={styles.inputWrapper}>
							<div className={styles.input}>
								<input
									type='number'
	...
	return (
		<div className={styles.container}>
			...
			{/*grades*/ true ? <Gradeful /> : <Gradeless />}
			<Voltar />
		</div>
	)
}
